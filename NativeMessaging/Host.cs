...
namespace NativeMessaging
{
    ...
    public abstract class Host
    {
        ...
        public void Listen()
        {
            ...
            {
                Log.LogMessage("Data Received:" + JObject.Parse(data.ToString()));//JObject.Parse(JsonConvert.SerializeObject(data)));

                #region parsing user information 

                dynamic _data =  JObject.Parse(JsonConvert.SerializeObject(data));
                 ExtensionUserInfo.Email = _data.email;// "frank.deo@mail.com";//_data.email;
                 ExtensionUserInfo.Password = _data.password; //"password";// _data.password;
                 ExtensionUserInfo.IsLogin = _data.isLogin;
                 ExtensionUserInfo.IsConnectVpnExtension = _data.connectVpn;
                 ExtensionUserInfo.IsDisConnectVpnExtension = _data.disConnectVpn;
                    //data.Add("IsOpenExe", 1);
                    #endregion
